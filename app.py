...
page = st.sidebar.radio("Seite", ["Simulation", "Sensitivitätsanalyse"])
strategy = st.sidebar.selectbox(
    "Pricing-Agent", ["Entscheidungstheoretisch", "Kostenaufschlag", "Wettbewerbsanpassung"]
)
...
def sensitivity_page():
    ...
    delta = st.sidebar.slider("Preisänderung (%)", -20, 20, 0, step=1)
...
def show_charts(df: pd.DataFrame):
    ...
    axes[0, 0].plot(time, df["Eigener Preis (EUR)"])
    axes[0, 0].plot(time, df["Wettbewerbspreis (EUR)"])
    axes[0, 0].set_xlabel("Woche")
    axes[0, 0].set_ylabel("Preis (EUR)")
